...
class TestHealthCheck:
    async def test_single_user_returns_settings_plan(self, mock_env, monkeypatch):
        """Single-user mode (no gateway identity) returns the global settings plan."""
        monkeypatch.delenv("JQUANTS_MCP_USER", raising=False)
        result = await _call("health_check")
        assert result["plan"] == "premium"
        assert result["api_key_configured"] is True

    async def test_multi_user_returns_user_plan(self, mock_env, monkeypatch):
        ...
        monkeypatch.setenv("JQUANTS_MCP_USER", "user-123")
        with patch.object(server_module, "_user_db", mock_db):
            result = await _call("health_check")

        assert result["plan"] == "standard"
        assert result["api_key_configured"] is True

    async def test_multi_user_unregistered_returns_default(self, mock_env, monkeypatch):
        """Unregistered OAuth user gets the global default plan."""
        mock_db = MagicMock()
        mock_db.get_user.return_value = None

        monkeypatch.setenv("JQUANTS_MCP_USER", "new-user")
        with patch.object(server_module, "_user_db", mock_db):
            result = await _call("health_check")
    ...
    async def test_no_user_db_returns_settings_plan(self, mock_env, monkeypatch):
        """OAuth user without encryption_key returns global plan."""
        monkeypatch.setenv("JQUANTS_MCP_USER", "user-456")
        with patch.object(server_module, "_user_db", None):
            result = await _call("health_check")

        assert result["plan"] == "premium"

    async def test_cache_ready_true_when_ok(self, mock_env, monkeypatch):
    ...
    async def test_cache_ready_false_when_pending(self, mock_env, monkeypatch):
    ...
    async def test_cache_ready_false_when_failed(self, mock_env, monkeypatch):
    ...
    async def test_latest_cache_date_none_when_empty(self, mock_env, monkeypatch):
        """latest_cache_date is None when equities_bars_daily has no rows."""
        monkeypatch.delenv("JQUANTS_MCP_USER", raising=False)
        result = await _call("health_check")
        assert result["latest_cache_date"] is None

    async def test_latest_cache_date_returns_max_date(self, mock_env, monkeypatch):
    ...
    async def test_trading_date_today_falls_back_to_weekday(self, mock_env, monkeypatch):
    ...
    async def test_trading_date_today_from_calendar(self, mock_env, monkeypatch):
    ...
    async def test_today_cache_ready_true_when_data_is_current(self, mock_env, monkeypatch):
    ...
    async def test_today_cache_ready_false_when_data_stale(self, mock_env, monkeypatch):
    ...
    async def test_today_cache_ready_false_when_cache_pending(self, mock_env, monkeypatch):
...
class TestCacheStatus:
    ...
    async def test_no_user_db_returns_cache_default_plan(self, mock_env, monkeypatch):

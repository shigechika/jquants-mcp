...
- `scripts/` — Operational scripts
  ...
  - `gcs_sync.py` — Cloud Run cache.db startup download from GCS (`--init-cache`). Skips the download (and the atomic replace that always allocates a new inode) when the GCS generation of the effective object -- whichever `cache.db.zst`/`cache.db` the zst-then-fallback precedence would actually use -- is unchanged since the last successful download, tracked in a `cache.db.generation.json` sidecar; this keeps `cache-poll.crontab`'s 15-minute poll from defeating `CacheStore`'s `(dev, ino)` integrity sidecar on every tick when nothing actually changed. Auth DBs are **no longer synced here**: `users.db`/`oauth_state.db` moved to Firestore on Cloud Run, so `_DOWNLOAD_FILES`/`_UPLOAD_FILES` are empty and `--init`/`--daemon` are auth-DB no-ops

...
Claude が `get_comparison_chart_data` を `mode="return_pct"`（デフォルト）で呼び、
...
| 質問 | ツール |
|---|---|
| 「今週の決算発表予定」（これからのスケジュール） | `get_earnings_this_week` |
| 「今週発表された決算実績」（実際の数値） | `get_earnings_results_this_week` |
| 「特定銘柄の次の決算日」 | `get_equities_earnings_calendar` |

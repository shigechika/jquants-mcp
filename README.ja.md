...
ユーザー向けドキュメントサイト: <https://shigechika.github.io/jquants-mcp/ja/>（5 分で始められる導入ガイド。英語版は [https://shigechika.github.io/jquants-mcp/](https://shigechika.github.io/jquants-mcp/)）。本 README は技術リファレンス（設定 schema、43 ツール全パラメータ表、デプロイ）です。
...
- **51 の MCP ツール** — J-Quants API v2 全エンドポイント（22）+ マーケット概況・バリュエーション（9）+ オフライン screener（9）+ テクニカル指標（1）+ 単銘柄サマリー（1）+ 銘柄名検索（1）+ チャートツール（3、JSON×2 常時登録 + PNG×1 opt-in）+ サーバーユーティリティ（5）
...
### マーケット概況・バリュエーション (Market Overview & Valuation) — 9ツール
...
> **チャートへの RSI 描画**: `render_candlestick` は現時点で RSI サブパネルを描画しません。RSI 数値は `get_technical_indicators` をご利用ください。

### チャートツール (Charts) — 3ツール

JSON ツール2本は常時登録（オプション依存なし）。PNG ツール1本は `[charts]` extra が必要。

**常時登録 JSON ツール** — React artifact / Plotly 向けの構造化データを返却:
...
**PNG ツール（opt-in）** — [`mplfinance`](https://github.com/matplotlib/mplfinance) + matplotlib が必要。Claude Desktop / モバイルでチャットに直接表示される。
...
| ツール名 | 説明 |
|---|---|
| `render_candlestick` | 単一銘柄の OHLC ローソク足 PNG。`get_candlestick_data` と同じ期間・インジケーターオプションに加え `style`・`aspect_ratio` 指定可。 |

共通インジケーターオプション（`get_candlestick_data` と `render_candlestick`）:

- **インジケーター**: `volume`, `sma5`, `sma20`, `sma25`, `sma60`, `sma75`, `sma200`, `bb20`（20日 Bollinger Band。JSON では `bb20_upper` / `bb20_mid` / `bb20_lower` に展開）
- **分割調整**: デフォルト `adjusted=True`、`False` で raw OHLC
- **スタイル**（`render_candlestick` のみ）: `default`（Yahoo 風）/ `dark` / `colorblind`（Okabe-Ito パレット）
- **アスペクト比**（`render_candlestick` のみ）: `square`（デフォルト, 8×8 in）/ `landscape`（12×6 in）/ `portrait`（6×9 in）

...
- **50 の MCP ツール** — J-Quants API v2 全エンドポイント（22）+ マーケット概況・バリュエーション（9）+ オフライン screener（9）+ テクニカル指標（1）+ 単銘柄サマリー（1）+ 銘柄名検索（1）+ チャートツール（2、JSON、オプション依存なし）+ サーバーユーティリティ（5）
...
本番の現行サイジング（[.github/workflows/cd.yml](.github/workflows/cd.yml) 参照）は `--memory 6Gi --cpu 2 --max-instances 3` で、CPU スロットリングはデフォルト（有効）のままです。CPU スロットリング有効＝**リクエストベース課金**で、リクエスト処理中だけ課金されるため、ほぼ idle な本サービスを月次フリーティア内に収められます。`--no-cpu-throttling` を付けるとインスタンスが生存している全秒（idle キープアライブ含む）が課金対象になります。4 GiB を超えるメモリ割り当てには Cloud Run gen2 が必要で、かつ >4 GiB は ≥2 vCPU が強制されます。

メモリは 6 GiB から下げないでください。cache reload は新スナップショットを `/tmp` の一時ファイルへダウンロードしてから旧ファイルへ rename するため、tmpfs のピークは一時的に旧＋新で約 2× `cache.db`（1× ではない）になります。約 3 GiB のスナップショットではこのピーク（約 2× `cache.db`）が 6 GiB 上限に近いので、`cache.db` が大きく成長した場合はメモリ上限を上げてください（かつ ≥2 vCPU を維持）。

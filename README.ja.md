...
メモリは 6 GiB から下げないでください。cache reload は新スナップショットを `/tmp` の一時ファイルへダウンロードしてから旧ファイルへ rename するため、tmpfs のピークは一時的に旧＋新で約 2× `cache.db`（1× ではない）になります。~2.9 GiB のスナップショットではこのピーク（~5.8 GiB）が既に 6 GiB 上限の近くにあるので、`cache.db` が大きく成長した場合はメモリ上限を上げてください（かつ ≥2 vCPU を維持）。

...
- **52 の MCP ツール** — J-Quants API v2 全エンドポイント（22）+ マーケット概況・バリュエーション（9）+ オフライン screener（10）+ テクニカル指標（1）+ 単銘柄サマリー（1）+ 銘柄名検索・決算カレンダー（2）+ チャートツール（2、JSON、オプション依存なし）+ サーバーユーティリティ（5）

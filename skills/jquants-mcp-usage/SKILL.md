...
- `summary` — advances/declines, 25-day advance/decline ratio, TOPIX change, market margin ratio.
...
get_stock_briefing(code="80530")        # price + FY financials + PER/PBR/yield + margin, one call
...
get_technical_indicators(code="80530", date_from="2026-06-15")
                                        ...
                                        # date_from/date_to required); add sma75 via indicators=[...]
...
- `get_top_movers` / `get_top_volume` / `get_top_turnover_value` — leaders by change, volume, value.
- `get_sector_performance` — 33-sector (or `sector_type="s17"`) daily change.
...
### Composite briefings (cache-only, start here)
...
- `detect_52w_high_low` / `detect_ytd_high_low` — fresh high/low breakouts on a date.
  These list stocks that SET a new high/low that day — for "sitting near the
  52-week low" use `get_value_stock_screen` instead.
...
- `get_technical_indicators` — numeric SMA/Bollinger/RSI for one stock
...
3. Single-user local mode uses `JQUANTS_API_KEY` env var or the `[jquants]` section in the config file

## Common Patterns

**Morning routine — one call answers "how is the market and what looks cheap":**
```text
health_check()                          # confirm cache_ready and plan
get_market_briefing(date="2026-07-14")  # includes value_screen + trend_signals
```

**Custom value screen when the default is too strict:**
```text
get_value_stock_screen(near_low_pct=15, max_pbr=1.5, require_profit_increase=False)
```

**Look up a company, then deep-dive:**
```text
search_equities(name="キオクシア")
get_stock_briefing(code="285A0")
get_candlestick_data(code="285A0", from_date="2026-04-01")
```

**Retrieve a week of screener results at once:**
```text
detect_52w_high_low_range(date_from="2026-07-07", date_to="2026-07-11")
```

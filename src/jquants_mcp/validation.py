...
# Re-validate the API key once per day
_VALIDATION_INTERVAL = 86400

# Evict in-memory clients idle for more than 1 hour
_STALE_CLIENT_TTL = 3600

# Probe endpoints for detecting the user's actual plan (highest plan first).
# Each tuple: (plan name, endpoint path, probe params)
# Verified against the J-Quants API v2 documentation:
#   /fins/details          → Premium only
#   /markets/short-ratio   → Standard / Premium (legacy path: /markets/short_selling)
#   /equities/investor-types → Light / Standard / Premium (legacy path: /markets/trades_spec)

...
_DATE_WITH_HYPHENS_RE = re.compile(
...
def validate_date(date: str | None, param: str = "date") -> str | None:
    ...
    if not _DATE_RE.match(stripped):
        return (
            f"'{param}' must be in YYYYMMDD or YYYY-MM-DD format. Got: '{date}'"
        )
    return None
...
def validate_sector33(code: str | None, param: str = "sector33_code") -> str | None:
    ...
    if code not in _VALID_SECTOR33_CODES:
        return (
            f"'{param}' must be a valid TSE 33-industry code (4-digit, e.g. '0050'). "
            f"Got: '{code}'"
        )
    return None
...
def validate_section(section: str | None, param: str = "section") -> str | None:
    ...
    if section not in _VALID_SECTIONS:
        return (
            f"'{param}' must be one of: {', '.join(sorted(_VALID_SECTIONS))}. "
            f"Got: '{section}'"
        )
    return None
